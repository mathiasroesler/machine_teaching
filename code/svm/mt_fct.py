...
def create_teacher_set(train_data, train_labels, test_data, test_labels, lam_coef, set_limit):
    ...
    model, teaching_data, teaching_labels = teacher_initialization(model, train_data, train_labels, positive_index, negative_index)
    ...
    while len(teaching_data) != nb_examples and len(teaching_data) < set_limit:
        ...
        curr_accuracy = update_model(model, teaching_data, teaching_labels, test_data, test_labels)
...
def train_student_model(train_data, train_labels, test_data, test_labels, max_iter=5000):
    ...
    model = student_model(max_iter=max_iter) # Declare student model
    
    if model == None:
        print("Error in function train_svm_model: the model was not created.")
        return None

    print("Training with ", len(train_data), " examples")
...
def update_teaching_set(teaching_data, teaching_labels, data, labels, added_indices):
    ...
    Input:  teaching_data -> np.array[np.array[int]], list of examples.
                First dimension number of examples.
                Second dimension features.
...
def update_model(model, teaching_data, teaching_labels, test_data, test_labels):
