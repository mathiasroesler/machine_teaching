...
def create_teacher_set(model_type, train_data, train_labels, test_data, test_labels, lam_coef, set_limit, batch_size=32, epochs=20):
    ...
    # Exit if all of the examples are in the teaching set
        ...
        if len(missed_indices) > 50:
            added_indices = rng.choice(missed_indices, 50, replace=False)
...
def teacher_initialization(model, model_type, train_data, train_labels, batch_size=32, epochs=20):
    ...
    positive_indices = np.where(train_labels != -1)
    positive_index = rng.choice(positive_indices[0])
    positive_example = train_data[rng.choice(positive_indices[0])] 
    
    # Find a random negative example
    negative_indices = np.where(train_labels == -1)
    negative_index = rng.choice(negative_indices[0])
    negative_example = train_data[rng.choice(negative_indices[0])]
